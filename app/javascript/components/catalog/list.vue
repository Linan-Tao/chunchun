  ...
  <li class="black-classify">
    ...
    <div class="search">
      <div class="input-group">
        ...
        <span class="input-group-addon"><i class="fa fa-2x fa-search"></i></span>
      </div>
    </div>
    <!-- list body -->
    <div class="list-body table-responsiev">
      <ul class="list-unstyled">
        <transition-group name="fade">
          <li class="column row ml-1 mt-1" v-for="(catalog, index) in filterCatalog(catalogs)" :data-id="catalog.id" :key="catalog.id"  v-on:mouseenter="mouseEnters(catalog)" v-on:mouseleave="mouseLeaves(catalog)" v-show="catalog.filterShow" :class="(catalog.selected ? ' active ' : '') + (isChecked ? ' p-0 ' : '')">
            <div class="col-sm-1 text-center" v-if="isChecked">
              <input type="checkbox" :data-id='catalog.id' @change='onChecked($el, catalog)'>
            </div>
            <div v-bind:class="hasEditableAndCheckbox()" @click="selected(catalog)">
              <i class="fa fa-circle text-info m-r-10 small"></i>{{catalog.name}}
            </div>
            <div class="col-sm-3 text-center text-success handle" v-show="editable">
              <span v-show="true">
  ...
  methods: {
    ...
    selected (catalog) {
      ...
      this.catalogs.forEach(function(value){
        ...
        value.children_data.forEach(function(value){
          value.selected = false;
        })
      })
    ...
    removeData (catalog, index) {
      if(confirm("确认删除？")){
        ...
        var errorHandler = function(response){
          alert(`删除失败: ${response.body.message}`);
        }
    ...
    addCatalog (obj) {
      ...
      var successHandler = function(response){
        ...
        if (this.options.addChildren) {// 分为内部添加按钮 和 底部添加按钮
          console.log(this.catalogs[options.index]);
          this.catalogs[options.index].children_data.push(reCatalog);
    ...
    onChecked (ele, catalog) {
      this.$emit('checked', {ele: ele, catalog: catalog})
    },
    hasEditableAndCheckbox () {
      if (this.editable) {
        if (this.isChecked) {
          return "col-sm-8"
        } else {
